...
async def fetch_candlestick(pair, session):
            ...
            query_params = {
                ...
                "to": (pd.Timestamp.now() + pd.Timedelta(hours=0, minutes=0)).timestamp(),
                "resolution": "30",  # '1' OR '5' OR '60' OR '1D'
                "pcode": "f"
            }
                ...
                if response.status == 200:
                    ...
                    if not data.empty:
                        ...
                        if data['signal'][1] > 1.0:
                            send_signal_message(pair,data['signal'][1],green_heart, "LONG")
                        elif data['signal'][1] < -1.0:
                                send_signal_message(pair, data['signal'][1], red_heart, "SHORT")
